...
if [ ! -z $REPO_NAME ]; then
    ...
    CMD="cd ${GIT_ROOT}/${REPO_NAME} ; git fetch ; git worktree add --checkout -b release/$BRANCH_NAME /home/hubo/workspace/git-depot/$BRANCH_NAME/$REPO_NAME origin/release/${BRANCH_NAME}"
    echo "$CMD"
    ...
    do
        cd ${GIT_ROOT}/${repo}
        CMD="git fetch ; git worktree add --checkout -b release/$BRANCH_NAME /home/hubo/workspace/git-depot/$BRANCH_NAME/$repo origin/release/${BRANCH_NAME}"
        echo "$CMD"
        read -p "Press any key to continue... " -n1 -s
