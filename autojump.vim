...
function! autojump#jumpdir(fragment)
  let s:path = autojump#complete(a:fragment)
  let s:dir = fnamemodify(s:path, ":p:h")
  echo "Change Directory:" s:dir "?"
  exec 'cd' s:dir
endfunction
...
" G jumps you to a directory
command! -nargs=1 -complete=customlist,autojump#completion G :call autojump#jumpdir(<f-args>)
